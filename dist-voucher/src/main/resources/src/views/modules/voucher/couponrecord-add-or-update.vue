<template>
  <el-dialog
    ...
    <el-form-item label="订单号" prop="businessId">
      <el-input v-model="dataForm.businessId" placeholder="订单号"></el-input>
    </el-form-item>
    ...
    <el-form-item label="截止日期" prop="validTime">
      <el-input v-model="dataForm.validTime" placeholder="截止日期"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/voucher/couponrecord/info/${this.dataForm.id}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.validTime = data.couponRecord.validTime
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/voucher/couponrecord/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
